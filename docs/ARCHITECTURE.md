...
| `mandrelView.js` | Lathe mesh; colour modes metal / flat (matte, user colour) / K / H (blue–green–red curvature map, auto or custom range, not tone-mapped); end caps, shaft; owns the mandrel group (pose matrix)                                                                                          |
...
| `colormaps.js`   | Palette: categorical slots, sequential indigo→magenta ramp (chosen by measured contrast against the rendered mandrel), blue–green–red curvature map of geo-framework (HSV hue 240°→120°→0°, green = 0), status colours                                                                   |
| `scales.js`      | Pure colour-scale logic: per-colouring value, unit and full range; data / custom / full ranges (sequential: centre or 0 as midpoint; curvature maps κn, κg, K, H: green at 0, each sign scaled separately, noise floor); slider bounds and ordering of custom bounds; data-range tracker |
